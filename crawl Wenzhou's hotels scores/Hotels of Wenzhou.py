...
def type(driver):
    ...
    for j in range(len(types)-1):
        type=driver.find_element_by_xpath('//*[@id="component_6"]/div/div[2]/div[5]/div[2]/div[{}]'.format(str(j+1)))
        ...
        if number !='0':
            type.click()
            time.sleep(5)
            pre_page=driver.find_element_by_xpath('//*[@id="taplc_dh_sort_filter_entry_0"]/div[1]/div[1]/div[1]/span').text
            print(pre_page)
            pre_page=re.findall(find_pre_page,pre_page)[0]
            print(pre_page)
            page=int(pre_page)//30
...
def get_page(driver):
    ...
    driver.get(url)
    for i in range(int(input('请输入页数（16页）：'))):
        ...
        with open('{}.txt'.format(str(i+1)),'w',encoding='utf-8') as f:
            f.write(html)
        if i==0:
            ActionChains(driver).double_click(driver.find_element_by_xpath('//*[@id="taplc_main_pagination_bar_dusty_hotels_resp_0"]/div/div/div/a')).perform()
        elif i==16:
            pass
        else:
...
def spider(link):
    ...
    rels=soup.find('div',class_="ui_columns hotels-hotel-review-about-with-photos-layout-LayoutStrategy__columns--1uvt4").find_all('div',class_="ui_column")


    if len(rels) >= 3:

        find_equip=re.compile('(?<="amenityNameLocalized":")(.*?)(?=")')
        comm=rels[0].find('span',class_="hotels-hotel-review-about-with-photos-Reviews__overallRating--vElGA").get_text()
        comm_num=rels[0].find('span',class_="hotels-hotel-review-about-with-photos-Reviews__seeAllReviews--3PpLR").get_text()
        ls.append(comm)
        ls.append(comm_num)
        ...
        try:
            features=soup.find_all('div',class_="hotels-hr-about-amenities-AmenityGroup__amenitiesList--3MdFn")[1].find_all('div',class_="hotels-hr-about-amenities-Amenity__amenity--3fbBj")
            fea_ls=[]
            for tag in features:
